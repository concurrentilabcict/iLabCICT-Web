    ...
    return(
        <article className="group flex h-full min-h-[300px] w-full max-w-[600px] cursor-pointer flex-col gap-3 rounded-3xl border border-white bg-white p-4 shadow-[0_14px_34px_rgba(15,23,42,0.08)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_42px_rgba(15,23,42,0.12)] md:max-w-[550px]">
                ...
                <div className="flex w-full gap-2">
                    ...
                    <ComputerArchiveAction computer={computer} queryKey={["technician-room-computers", room ?? ""]} />
...
function InfoTile({ icon: Icon, label, value, highlight = false }: InfoTileProps) {
    return (
        <div className={`flex min-w-0 items-center gap-2.5 rounded-2xl p-3 shadow-sm shadow-black/[0.01] ${highlight ? "bg-red-50/50" : "bg-zinc-50"}`}>
            <div className={`flex size-8 shrink-0 items-center justify-center rounded-xl bg-white ${highlight ? "primary-text-color" : "text-zinc-400"}`}>
                <Icon size={16} />
            </div>
